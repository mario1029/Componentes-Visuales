...
class Treeview extends VisualComponent {
  constructor({ id }) {
    ...
    this.wrapper.className = 'treeview_env_0'; //Opcion 0 por defecto
  ...
  setCSS() {
    this.wrapper.className = 'treeview_env_' + this.env;
  }
  ...
  addElement(rama, raiz) {
    for (let i in rama) {
      if (rama[i].Hijos != 0) {
        let content =
          document.createElement('li'); /*SE CREA UN ELEMNTO DE LISTA "li"*/
        content.innerHTML = `<strong> ${rama[i].Nombre} </strong>`; /*SE AÑADE NOMBRE DE CARPETA*/

        console.log(rama[i]);

        let nuevaraiz =
          document.createElement('ul'); /*SE CREA UNA NUEVA LITSA "nuevaraiz"*/
        ...
        this.setViewState(
          content,
          nuevaraiz
        ); /*SE CREAR EL EVENTO DE LA LISTA DESPLEGABLE*/
